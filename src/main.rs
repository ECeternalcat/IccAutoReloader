...
fn main() -> Result<()> {
    ...
    unsafe {
        ...
        let h_icon = LoadIconW(instance, PCWSTR(1 as *const u16)).unwrap_or_else(|_| LoadIconW(None, IDI_APPLICATION).unwrap());
...
fn load_settings() {
    if let Some(val) = get_reg_dword(w!("PollingInterval")) {
        if val >= 1000 && val <= 10000 { POLLING_INTERVAL_MS.store(val, Ordering::Relaxed); }
    }
    if let Some(val) = get_reg_dword(w!("Language")) {
        if val <= 2 { CURRENT_LANG.store(val as usize, Ordering::Relaxed); }
    }
}
...
fn is_polling_enabled() -> bool {
    if let Some(val) = get_reg_dword(w!("PollingEnabled")) { return val != 0; }
    true
}
        ...
        if RegOpenKeyExW(HKEY_CURRENT_USER, REG_RUN_KEY, 0, KEY_WRITE, &mut key).is_ok() {
            if enabled {
                ...
                if let Ok(exe_path) = std::env::current_exe() {
                    ...
                    let _ = RegSetValueExW(key, APP_NAME, 0, REG_SZ, Some(std::slice::from_raw_parts(path_wide.as_ptr() as *const u8, path_wide.len() * 2)));
                }
            }
            RegCloseKey(key);
        }
...
fn check_first_run(window: HWND) {
    ...
    unsafe {
        ...
        if MessageBoxW(window, msg, APP_NAME, MB_YESNO | MB_ICONQUESTION) == IDYES {
    ...
    unsafe {
        ...
        for channel in 0..3 {
            for i in 0..256 {
                ...
                if (ramp[channel][i] as i32 - expected).abs() > tolerance { return false; }
            }
        }
        true
    }
...
fn reload_profile() {
    ...
    if let Some(path) = dispwin_path {
        let _ = Command::new(path).arg("-L").creation_flags(0x08000000).spawn();
    }
}
            ...
            if dialog.Show(window).is_ok() {
                if let Ok(result) = dialog.GetResult() {
                    if let Ok(path) = result.GetDisplayName(SIGDN_FILESYSPATH) {
                        if let Ok(path_str) = path.to_string() { install_profile(&path_str); }
                    }
                }
            }
        } else {
...
fn install_profile(path_str: &str) {
    ...
    if let Some(exe) = dispwin_path {
        let status = Command::new(&exe).arg("-I").arg(path_str).creation_flags(0x08000000).status();
        if let Ok(s) = status {
            if s.success() {
                reload_profile();
                unsafe { MessageBoxW(None, get_str("msg_install_success"), APP_NAME, MB_OK | MB_ICONINFORMATION); }
            } else {
                 unsafe { MessageBoxW(None, get_str("msg_install_fail"), APP_NAME, MB_OK | MB_ICONERROR); }
            }
        }
    } else {
        unsafe { MessageBoxW(None, get_str("msg_dispwin_missing"), APP_NAME, MB_OK | MB_ICONERROR); }
    }
}
...
fn find_dispwin() -> Option<PathBuf> {
    ...
    for c in &candidates {
        if c.exists() && c.file_name().is_some() { return Some(c.clone()); }
    }
    None
}
...
unsafe fn add_tray_icon(window: HWND) -> Result<()> {
    ...
    let instance = GetModuleHandleW(None)?;
    nid.hIcon = LoadIconW(instance, PCWSTR(1 as *const u16)).unwrap_or_else(|_| LoadIconW(None, IDI_APPLICATION).unwrap());

    let tip = "Icc Auto Reloader\0";
    let tip_wide: Vec<u16> = tip.encode_utf16().collect();
    if tip_wide.len() <= 128 {
        for (i, &b) in tip_wide.iter().enumerate() {
            nid.szTip[i] = b;
        }
    }
...
unsafe fn show_context_menu(window: HWND) {
    ...
    let startup_flags = if is_startup_enabled() { MF_CHECKED | MF_STRING } else { MF_STRING };
    let polling_flags = if is_polling_enabled() { MF_CHECKED | MF_STRING } else { MF_STRING };

    let _ = AppendMenuW(hmenu, MF_STRING, IDM_RELOAD, get_str("tray_reload"));
    let _ = AppendMenuW(hmenu, MF_STRING, IDM_IMPORT, get_str("tray_import"));
    let _ = AppendMenuW(hmenu, MF_SEPARATOR, 0, None);
    let _ = AppendMenuW(hmenu, polling_flags, IDM_POLLING, get_str("tray_polling"));
    let _ = AppendMenuW(hmenu, startup_flags, IDM_STARTUP, get_str("tray_startup"));
    let _ = AppendMenuW(hmenu, MF_STRING, IDM_SETTINGS, get_str("tray_settings"));
    let _ = AppendMenuW(hmenu, MF_SEPARATOR, 0, None);
    let _ = AppendMenuW(hmenu, MF_STRING, IDM_EXIT, get_str("tray_exit"));

    SetForegroundWindow(window);
    TrackPopupMenu(hmenu, TPM_BOTTOMALIGN | TPM_LEFTALIGN, point.x, point.y, 0, window, None);
